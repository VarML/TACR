...
def evaluate(
    ...
    for t in count():
        ...
        for i in ss:
            ...
            l.append(i.tolist())
        l = np.array(l)
        ss = torch.from_numpy(l).reshape(-1, state_dim).to(device=device, dtype=torch.float32)

        action = model.get_action(
            ss.to(dtype=torch.float32),
            actions.to(dtype=torch.float32),
            rewards.to(dtype=torch.float32),
            target_return.to(dtype=torch.float32),
            timesteps.to(dtype=torch.long),
    ...
    traj = {"observations": obs, "next_state":next,"rewards": rews, "dones": news, "actions": acs}
    episode_data = {}
    paths = []
    for k in traj:
