...
class SequenceTrainer(Trainer):
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            target_Q1, target_Q2 = self.critic_target(next_state, next_Q_action_preds)
            target_Q = torch.min(target_Q1)
            target_Q = rewards + dones * self.discount * target_Q

        current_Q1, current_Q2 = self.critic(states, action_sample)
        # Compute critic loss
        ...
        torch.nn.utils.clip_grad_norm_(self.actor.parameters(), .35)        
        self.optimizer.step()
